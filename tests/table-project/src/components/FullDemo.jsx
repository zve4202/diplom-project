...
function FullDemo() {
    ...
    const [savedModules, setSavedModules] = useSessionState(
        "savedModules",
        saveModules.Items | saveModules.SortOrder
    );
    const [columnVisibility, setColumnVisibility] = useSessionState(
        "columnVisibility",
        {}
    );

    const [restoredColWidths, saveColWidths] = useSessionValue(
        "columnWidths",
        {}
    );
        ...
        (e, selection) => {
            switch (e.keyCode) {
                case 46: //Delete
                    ...
                    if (options.multiSelect) actions.deleteItems(...selection);
                    else actions.deleteItems(selection);
                    break;
                default:
                    break;
            }
    ...
    return (
        <>
            ...
            <div className="controls">
                <button onClick={() => actions.setItems(todos)}>
                    Set items
                </button>
                ...
                <button
                    onClick={() => actions.setError("Something went wrong")}
                >
                    Set error
                </button>
                <button onClick={() => actions.startLoading()}>
                    Set loading
                </button>
                <div className="break" />
                ...
                <button
                    onClick={() => {
                        const selectedKeys = setUtils.getItems(
                            getState().selected
                        );
                        const patch = _.zipObject(
                            selectedKeys,
                            selectedKeys.map(() => ({ completed: true }))
                        );
                        actions.patchItemsByKey(patch);
                    }}
                >
                    Set selected items completed
                </button>

                <button
                    onClick={() =>
                        actions.patchItemsByKey({
                            200: { id: 195 },
                            195: { id: 200 }
                        })
                    }
                >
                    Swap values of items 195 and 200
                </button>
                ...
                >
                    Change title of item 193 and set 194 completed
                </button>
                ...
                <button
                    onClick={() =>
                        actions.addItems(
                            {
                                id: 201,
                                title: "Download react-select-table",
                                completed: true,
                                userId: 1
                            },
                            {
            ...
            <div className="controls">
                <button
                    onClick={() => actions.setItemFilter({ completed: false })}
                >
                    Only show non-completed
                </button>
                <button onClick={() => actions.setItemFilter({})}>
                    Clear filter
                </button>
            </div>
            <h3>Pagination</h3>
            <div className="controls">
                <Input
                    ...
                    min="0"
                />
                <button onClick={() => actions.setPageSize(pageSize || 0)}>
                    Apply
                </button>
            </div>
            ...
            <div className="controls">
                ...
                <button
                    onClick={() =>
                        applyOptions({
                            savedState: selectors.getSaveState(
                                getState(),
                                savedModules
                            )
                        })
            ...
            <div className="controls">
                <Checkbox
                    ...
                    onChange={(checked) =>
                        applyOptions({ multiSelect: checked })
                    }
                />
                <Checkbox
                    id="listBox"
                ...
                <Checkbox
                    ...
                    onChange={(checked) =>
                        applyOptions({ constantWidth: checked })
                    }
                />
            </div>
